...
function setPreflight(){
  if(BluetoothManager.preflight()){
    ...
    el.preflight.classList.add('success');
  }else{
    el.preflight.textContent='Web Bluetooth nicht unterstützt';
    el.preflight.classList.add('error');
  }
}

function setConnectedUI(yes){
  el.connect.disabled=yes; el.disconnect.disabled=!yes; el.state.textContent=yes?'Verbunden':'Getrennt';
}
...
function renderExplorer(tree){
  ...
  for(const svc of tree){
    ...
    for(const c of svc.characteristics){
      ...
      row.style.display='flex'; row.style.justifyContent='space-between'; row.style.gap='.5rem'; row.style.alignItems='center';
      ...
      act.append(br);
      row.append(lt,act); inner.append(row);
    }
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  el.connect.addEventListener('click', async ()=>{
    ...
    }catch(e){ log(el.log,'ERROR', e.message); }
  });
